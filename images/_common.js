$(function () {
  ...
  $("body").bind("click", function (e) {
    ...
    if ($target.closest(".btn_menu").length > 0) {
      ...
      $target.closest(".area_search").length == 0 &&
      $target.closest(".area_menu").length == 0
    ) {
      $("#wrap").removeClass("menu_on");
  ...
  if ($visitCounter.length == false) {
    ...
    $boxVisit
      .find(".data_today .item_visit")
      .text($visitCounter.find(".today").text());
    $boxVisit
      .find(".data_yesterday .item_visit")
      .text($visitCounter.find(".yesterday").text());
    $boxVisit
      .find(".data_total .item_visit")
      .text($visitCounter.find(".total").text());
  }
  ...
  $(".index_type_common .btn_post_more").bind("click", function () {
    ...
    if ($container.find(".index_list_container.list_gallery").length > 0) {
      $container
        .find(".index_list_container li.list_gallery_item:hidden")
        .each(function (i) {
          var $o = $(this);
          if (i < 6) {
            $o.slideDown();
    ...
    if (
      $container.children(".index_list_container").children("li:hidden")
        .length == 0
    ) {
      $btn.hide();
    }
  ...
  $(".category_list.index_type_common .article_content .thumbnail_post").each(
    function (i) {
      var path = $(this)
